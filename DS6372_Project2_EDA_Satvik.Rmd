...
bank_full <- read.csv("bank-additional-full.csv", header=TRUE, sep = ';',stringsAsFactors = F)
...
```{r}
library(ggplot2)
#Predictors that have large negative values push samples left
#Predictors with large positive values push samples right
loading_scores <- pc.result$rotation[,4]
```
...
model.main<-glm(y ~ ., data=o.newBankAdd,family = binomial)
summary(model.main)
